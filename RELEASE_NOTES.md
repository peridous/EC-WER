...
| `scripts/train_edit_conditioned_classifier.py` | From `scripts/experiments/train_edit_conditioned_classifier.py`; default seed changed from 20260802 to 20260807 to match the saved EC run. Explicit `fix_mistral_regex=False` preserves the training Metaspace encoding in online and offline loading. Objective and training logic are unchanged. |
...
| `tests/test_ecwer_tokenizer.py` | Checks the explicit training tokenizer setting and training token-ID fingerprint; rejects altered preprocessing or untested tokenizer-library versions. Optional artifact-backed regression compares all 3,626 validation encodings with the original serialized training backend in online/offline runs. |
